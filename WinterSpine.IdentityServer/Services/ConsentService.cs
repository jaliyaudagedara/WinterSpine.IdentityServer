...
namespace WinterSpine.IdentityServer.Services
{
    public class ConsentService
    {
        ...
        public async Task<ProcessConsentResult> ProcessConsent(ConsentInputModel model)
        {
            ...
            else if (model.Button == "yes" && model != null)
            {
                ...
                if (model.ScopesConsented != null && model.ScopesConsented.Any())
                {
                    ...
                    if (ConsentOptions.IsEnableOfflineAccess == false)
                    {
                    ...
                    {
                        RememberConsent = model.IsRememberConsent,
        ...
        private ConsentViewModel CreateConsentViewModel(ConsentInputModel model, string returnUrl, AuthorizationRequest request, Client client, Resources resources)
        {
            ...
            vm.IsRememberConsent = model?.IsRememberConsent ?? true;
            ...
            vm.IsAllowRememberConsent = client.AllowRememberConsent;

            vm.IdentityScopes = resources.IdentityResources.Select(x => CreateScopeViewModel(x, vm.ScopesConsented.Contains(x.Name) || model == null)).ToArray();
            vm.ResourceScopes = resources.ApiResources.SelectMany(x => x.Scopes).Select(x => CreateScopeViewModel(x, vm.ScopesConsented.Contains(x.Name) || model == null)).ToArray();
            if (ConsentOptions.IsEnableOfflineAccess && resources.OfflineAccess)
            {
        ...
        public ScopeViewModel CreateScopeViewModel(IdentityResource identity, bool check)
        {
            return new ScopeViewModel
            {
                ...
                IsEmphasize = identity.Emphasize,
                IsRequired = identity.Required,
                IsChecked = check || identity.Required,
            };
        }
        ...
        public ScopeViewModel CreateScopeViewModel(Scope scope, bool check)
        {
            return new ScopeViewModel
            {
                ...
                IsEmphasize = scope.Emphasize,
                IsRequired = scope.Required,
                IsChecked = check || scope.Required,
            };
        }
